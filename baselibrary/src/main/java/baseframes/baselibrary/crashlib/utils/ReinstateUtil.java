...
public class ReinstateUtil {
    ...
    private ReinstateUtil() {
        throw new ReinstateException("Stub!");
    }

    public static <T> T checkNotNull(T t, String message) {
        if (t == null)
            throw new ReinstateException(String.valueOf(message));
        return t;
    }
    ...
    private static File getDataDir() {
        return new File(File.separator + "data" + File.separator + "data" + File.separator + Reinstate.getInstance().getContext().getPackageName());
    }

    private static File getExternalDataDir() {
        File file = null;
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            file = baseframes.baselibrary.crashlib.rulers.Reinstate.getInstance().getContext().getExternalCacheDir();
        }
    ...
    private static boolean clearAppData(File dir) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (file.isFile() && file.exists()) {
                ...
                ReinstateLog.e(file.getName() + (result ? " delete success!" : " delete failed!"));
                continue;
            }
