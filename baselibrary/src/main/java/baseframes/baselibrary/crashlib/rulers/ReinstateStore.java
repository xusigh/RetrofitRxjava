 ...
 * 恢复的activity的管理
 */
public final class ReinstateStore {
    ...
    private volatile static ReinstateStore sInstance;

    private static final Object LOCK = new Object();
    //将所有打开的activity 都放入一个集合中
    private List<WeakReference<Activity>> mRunningActivities;
    //标记当前的activity
    private Intent mIntent;

    private ReinstateStore() {
        mRunningActivities = new CopyOnWriteArrayList<>();
    }

    public static ReinstateStore getInstance() {
        if (sInstance == null) {
            synchronized (LOCK) {
                if (sInstance == null) {
                    sInstance = new ReinstateStore();
                }
            }
        }
        return sInstance;
    ...
    public boolean verifyActivity(Activity activity) {
        return activity != null && !baseframes.baselibrary.crashlib.rulers.Reinstate.getInstance().getSkipActivities().contains(activity.getClass()) && !ReinstateActivity.class.isInstance(activity);
    }
