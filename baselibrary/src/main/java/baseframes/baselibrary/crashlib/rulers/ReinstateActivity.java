 ...
 * cash时显示的activity
 */
public final class ReinstateActivity extends AppCompatActivity {
    //标记
    ...
    private void setupToolbar() {
        ...
        mToolbar.setTitle(ReinstateUtil.getAppName(this));
    ...
    private void initView() {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            mScrollView.setPadding(0, ReinstateUtil.dp2px(getApplication(), 16), 0, 0);
        }
    }
    ...
    private void setupEvent() {
        mRecoverBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                boolean restart = ReinstateSharedPrefsUtil.shouldRestartApp();
                if (restart) {
                    ReinstateSharedPrefsUtil.clear();
                    restart();
                    return;
                }
                if (isRecoverStack()) {
        ...
        mRestartClearBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                AlertDialog dialog = new AlertDialog.Builder(ReinstateActivity.this)
        ...
        mCrashTipsTv.setText(String.format(getResources().getString(R.string.recovery_crash_tips_msg), ReinstateUtil.getAppName(this)));
    ...
    private boolean isDebugMode() {
        return getIntent().getBooleanExtra(ReinstateStore.IS_DEBUG, false);
    }

    private ReinstateStore.ExceptionData getExceptionData() {
        return getIntent().getParcelableExtra(ReinstateStore.EXCEPTION_DATA);
    }

    private String getCause() {
        return getIntent().getStringExtra(ReinstateStore.EXCEPTION_CAUSE);
    }

    private String getStackTrace() {
        return getIntent().getStringExtra(ReinstateStore.STACK_TRACE);
    }
    ...
    private boolean isRecoverStack() {
        boolean hasRecoverStack = getIntent().hasExtra(ReinstateStore.RECOVERY_STACK);
        return !hasRecoverStack || getIntent().getBooleanExtra(ReinstateStore.RECOVERY_STACK, true);
    }
    //点击了恢复，但是crash已经发生两次及其以上，将出新的activity栈
    ...
    private Intent getRecoveryIntent() {
        boolean hasRecoverIntent = getIntent().hasExtra(ReinstateStore.RECOVERY_INTENT);
        if (!hasRecoverIntent)
            return null;
        return getIntent().getParcelableExtra(ReinstateStore.RECOVERY_INTENT);
    }

    private ArrayList<Intent> getRecoveryIntents() {
        boolean hasRecoveryIntents = getIntent().hasExtra(ReinstateStore.RECOVERY_INTENTS);
        if (!hasRecoveryIntents)
            return null;
        return getIntent().getParcelableArrayListExtra(ReinstateStore.RECOVERY_INTENTS);
    }

    private boolean saveCrashData() {
        String date = ReinstateUtil.getDateFormat().format(new Date(System.currentTimeMillis()));
